...
function completionStatus(m) {
  const a = m.classes.filter(Boolean).length;
  if (m.appComplete && a === 4 && m.luncheonAttended) return 'done';
  if (a > 0 || m.photoTaken || m.luncheonAttended) return 'prog';
  return 'new';
}
...
const emptyMember = () => ({
  firstName:'', lastName:'', enrollDate: todayStr(),
  classes:[false,false,false,false],
  photoTaken:false, appComplete:false, luncheonAttended:false,
  assignedTo:'', notes:'', initials:''
});
...
export default function App() {
  ...
  const [modal, setModal] = useState(null); // 'member' | 'import' | 'assignees'
  ...
  function toggleClass(id, ci) {
    setMembers(prev => prev.map(m => {
      if (m.id !== id) return m;
      const classes = [...m.classes]; classes[ci] = !classes[ci];
      return { ...m, classes };
    }));
  }
  ...
  function openAdd() {
    setEditId(null); setForm(emptyMember()); setModal('member');
  }
  function openEdit(id) {
    const m = members.find(x => x.id === id);
    setEditId(id); setForm({ ...m, classes: [...m.classes] }); setModal('member');
  }
  function saveMember() {
  ...
  function closeModal() {
    setModal(null); setIStep(1); setCsvHeaders([]); setCsvRows([]); setIPreview([]); setIResult(null);
    setShStep(1); setShData([]); setShResult(null); setShLoading(false); setNewAssignee('');
  }

  function addAssignee() {
    const n = newAssignee.trim();
    if (!n || assignees.includes(n)) return;
    setAssignees(prev => [...prev, n]); setNewAssignee('');
  }
  function removeAssignee(i) {
    const name = assignees[i];
    if (members.some(m => m.assignedTo === name)) {
      if (!confirm(`${name} is assigned to members. Remove anyway? Their assignments will be cleared.`)) return;
      setMembers(prev => prev.map(m => m.assignedTo === name ? { ...m, assignedTo: '' } : m));
    }
  ...
  function exportCSV() {
    ...
    filtered.forEach(m => {
      rows.push([m.firstName, m.lastName, m.enrollDate,
        ...m.classes.map(c => c ? 'Y' : 'N'),
        m.classes.filter(Boolean).length,
        m.photoTaken ? 'Y' : 'N', m.appComplete ? 'Y' : 'N', m.luncheonAttended ? 'Y' : 'N',
        m.assignedTo || '', m.notes || '', m.initials || '', completionLabel(m)]);
    });
  ...
  function buildPreview() {
    const prev = csvRows.slice(0, 5).map(row => {
      ...
      const dup = members.some(m => m.firstName.toLowerCase() === fn.toLowerCase() && m.lastName.toLowerCase() === ln.toLowerCase());
  ...
  function doImport() {
    ...
    csvRows.forEach(row => {
      ...
      if (members.some(m => m.firstName.toLowerCase() === fn.toLowerCase() && m.lastName.toLowerCase() === ln.toLowerCase())) { skipped++; return; }
      newMembers.push({ id: id++, firstName: fn, lastName: ln, enrollDate: (row[csvMap.ed] || '').trim() || todayStr(), classes: [false,false,false,false], photoTaken: false, appComplete: false, luncheonAttended: false, assignedTo: '', notes: '', initials: '' });
      added++;
    });
    setMembers(prev => [...prev, ...newMembers]);
    setNextId(id);
    setIResult({ added, skipped }); setIStep(4);
  }
  ...
  function doShelbyImport() {
    let added = 0, skipped = 0;
    const newMembers = [];
    let id = nextId;
    shData.forEach(r => {
      if (r.dup || !r.fn || !r.ln) { skipped++; return; }
      newMembers.push({ id: id++, firstName: r.fn, lastName: r.ln, enrollDate: r.ed || todayStr(), classes: [false,false,false,false], photoTaken: false, appComplete: false, luncheonAttended: false, assignedTo: '', notes: '', initials: '' });
      added++;
    });
    setMembers(prev => [...prev, ...newMembers]);
    setNextId(id);
    setShResult({ added, skipped }); setShStep(3);
  }
        ...
        <table>
          <thead>
            <tr>
              <th>Member name</th>
              <th>Enrolled</th>
              <th className="center">C1</th>
              <th className="center">C2</th>
              <th className="center">C3</th>
              <th className="center">C4</th>
              <th className="center">Photo</th>
              <th className="center">App</th>
              <th className="center">Luncheon</th>
              <th>Assigned to</th>
              <th>Notes</th>
              <th className="center">Initials</th>
              <th>Status</th>
              <th></th>
            </tr>
          </thead>
            ...
            <div className="form-row">
              ...
              <div className="dot-row">
                {form.classes.map((c, i) => (
                  <button key={i} className={`dot dot-lg ${c ? 'dot-on' : 'dot-off'}`} onClick={() => setForm(f => { const classes = [...f.classes]; classes[i] = !classes[i]; return { ...f, classes }; })}>C{i+1}</button>
                ))}
              </div>
            </div>
          ...
          <div className="modal modal-wide">
            ...
            {iTab === 'csv' && (<>
              ...
              {iStep === 3 && (<>
                ...
                <table className="preview-table">
                  ...
                  <tbody>{iPreview.map((r, i) => (<tr key={i} className={r.dup ? 'dup-row' : ''}><td>{r.fn || '—'}</td><td>{r.ln || '—'}</td><td>{r.ed}</td><td className={r.dup ? 'dup-text' : 'new-text'}>{r.dup ? 'Duplicate — skip' : 'New'}</td></tr>))}</tbody>
                </table>
                <div className="modal-footer modal-footer-split">
                  <button className="btn" onClick={() => setIStep(2)}>← Back</button>
                  <button className="btn btn-success" onClick={doImport}>Import {csvRows.length} members</button>
                </div>
              </>)}
              {iStep === 4 && (<>
            ...
            {iTab === 'shelby' && (<>
              {shStep === 1 && (<>
                ...
                <div className="modal-footer">
                  <button className="btn" onClick={closeModal}>Cancel</button>
                  <button className="btn btn-primary" onClick={doShelbyFetch} disabled={shLoading}>{shLoading ? 'Fetching...' : '🔌 Connect & fetch members'}</button>
                </div>
              </>)}
              {shStep === 2 && (<>
                ...
                <div className="modal-footer modal-footer-split">
                  ...
                  <button className="btn btn-success" onClick={doShelbyImport}>Import {shData.filter(r => !r.dup).length} new members</button>
                </div>
              </>)}
              {shStep === 3 && (<>
                {shResult?.err ? (
