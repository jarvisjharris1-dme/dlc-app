...
export default function ConnectTeamShell({ onHome }){
  ...
  async function saveItem(){
    if(!editing) return;
    const {error} = await supabase.from('connect_team_inventory').update({item_name:editing.item_name,category:editing.category,quantity:Number(editing.quantity)||0,minimum_quantity:Number(editing.minimum_quantity)||0,notes:editing.notes || '',updated_at:new Date().toISOString()}).eq('id',editing.id);
  ...
  return <div className="ct-shell-wrap">
    ...
    {inventoryOpen && <div className="ct-modal-bg" onClick={e=>e.target===e.currentTarget&&setInventoryOpen(false)}><div className="ct-inventory-modal"><div className="ct-modal-head"><div><small>CONNECT TEAM</small><h2>Inventory</h2><p>Keep new-member materials ready.</p></div><button onClick={()=>setInventoryOpen(false)}>✕</button></div>{message && <div className="ct-message">{message}<button onClick={()=>setMessage('')}>×</button></div>}<div className="ct-add-grid"><input placeholder="Item name" value={newItem.item_name} onChange={e=>setNewItem({...newItem,item_name:e.target.value})}/><select value={newItem.category} onChange={e=>setNewItem({...newItem,category:e.target.value})}><option>New Member Materials</option><option>Class Materials</option><option>Completion Materials</option><option>Office Supplies</option><option>Other</option></select><input type="number" min="0" placeholder="On hand" value={newItem.quantity} onChange={e=>setNewItem({...newItem,quantity:e.target.value})}/><input type="number" min="0" placeholder="Minimum" value={newItem.minimum_quantity} onChange={e=>setNewItem({...newItem,minimum_quantity:e.target.value})}/><button className="ct-primary" onClick={addItem}>+ Add Item</button></div><div className="ct-table-wrap"><table><thead><tr><th>Item</th><th>On Hand</th><th>Minimum</th><th>Status</th><th>Quick Update</th><th></th></tr></thead><tbody>{items.map(item=><tr key={item.id}><td><strong>{item.item_name}</strong><br/><small>{item.category || 'Other'}</small></td><td>{item.quantity}</td><td>{item.minimum_quantity}</td><td><span className={`ct-status ${item.quantity<=item.minimum_quantity?'low':'good'}`}>{item.quantity===0?'Out of Stock':item.quantity<=item.minimum_quantity?'Low Stock':'Good'}</span></td><td><button onClick={()=>quickAdjust(item,5)}>+5</button> <button onClick={()=>quickAdjust(item,-5)}>-5</button></td><td><button onClick={()=>setEditing({...item})}>Update</button> <button className="ct-danger" onClick={()=>deleteItem(item)}>Delete</button></td></tr>)}{!items.length&&<tr><td colSpan="6" className="ct-empty">No inventory items yet.</td></tr>}</tbody></table></div></div></div>}
